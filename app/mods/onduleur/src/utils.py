...
def getDataFromBddInBetweenTstmp(start, end):
    data = mysqlite.exec("select * from data where strftime('%s', time) > ? and strftime('%s', time) < ? order by time desc", (str(start), str(end)))
    return formatAllData(data)
