...
function createList(){
    ...
    for (let ip in onduleurs){
        ...
        lien.setAttribute("href", "/onduleur?ip=" + ip + "&port=" + onduleurs[ip]["port"] + "&slave_id=" + onduleurs[ip]["slave_id"])
        lien.innerText = ip
        p.appendChild(lien)
...
function formSubmit(event) {
    ...
    window.location.href="/onduleur?ip=" + values["ip"] + "&port=" + values["port"] + "&slave_id=" + values["slave_id"];
}
